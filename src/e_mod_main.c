   ...
   if (action_handler_key_down)
    {
       ...
       action_handler_key_down = NULL;
    }
   if (action_handler_key_up)
    {
       ecore_event_handler_del(action_handler_key_up);
       action_handler_key_up = NULL;
    }
}

static Eina_Bool
_e_text_input_method_context_keyboard_grab_key_down(void *data EINA_UNUSED, int type EINA_UNUSED, void *event)
{
...
static Eina_Bool
_e_text_input_method_context_keyboard_grab_key_up(void *data EINA_UNUSED, int type EINA_UNUSED, void *event)
{
   E_Input_Method_Context *context = NULL;
   E_Comp_Data *cdata = NULL;
   Ecore_Event_Key *ev;
...
_e_text_input_method_context_cb_keyboard_grab(struct wl_client *client, struct wl_resource *resource, uint32_t id)
{
   ...
   if (!action_handler_key_down)
     action_handler_key_down = ecore_event_handler_add(ECORE_EVENT_KEY_DOWN, _e_text_input_method_context_keyboard_grab_key_down, context);
   if (!action_handler_key_up)
     action_handler_key_up = ecore_event_handler_add(ECORE_EVENT_KEY_UP, _e_text_input_method_context_keyboard_grab_key_up, context);
}
