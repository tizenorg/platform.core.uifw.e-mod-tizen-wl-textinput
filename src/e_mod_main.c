...
static Eina_Bool g_keyboard_mode_latin = EINA_TRUE;
...
_display_language_changed_cb(keynode_t *key, void* data)
{
   ...
   g_keyboard_mode_latin = EINA_TRUE;
}
...
static Eina_Bool
_e_text_input_method_context_filter_hotkeys(Ecore_Event_Key *ev)
{
   ...
   if (strncmp(ev->keyname, keyname_space, strlen(keyname_space)) == 0 && ev->modifiers & ECORE_EVENT_MODIFIER_SHIFT)
     {
        if (g_keyboard_mode_latin)
          {
             e_comp_wl_input_keymap_set(g_keymap_info[g_keymap_index].rules, g_keymap_info[g_keymap_index].models, g_keymap_info[g_keymap_index].layout, NULL, NULL, NULL, NULL);
             g_keyboard_mode_latin = EINA_FALSE;
          }
        else
          {
             e_comp_wl_input_keymap_set(g_keymap_info[0].rules, g_keymap_info[0].models, g_keymap_info[0].layout, NULL, NULL, NULL, NULL);
...
_e_text_input_method_context_ecore_cb_key_down(void *data, int ev_type EINA_UNUSED, Ecore_Event_Key *ev)
{
   ...
   if (!_e_text_input_method_context_filter_hotkeys(ev))
     {
